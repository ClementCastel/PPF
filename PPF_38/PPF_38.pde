...
boolean combat_boss = false;
boolean boss_generate = false;
float boss_x, boss_y;
int pirate_T_x = 80;
int pirate_T_y = 450;
boolean turn_boss = false;
int player_sante = 200;
int boss_sante = 2500;
boolean player_animate;
...
void setup () {
  ...
  g4_map = loadImage("data/ile_principale.JPG");
...
void draw () {
  ...
  if (gameStatement == 4) { //ILE PRINCIPALE
    ...
    if (dist(g4_x, g4_y, -570, -130) <= 100) { //VERIFICATION CIRCULAIRE DISTANCE PERSONNAGE / 1ER SHOP
    ...
    if (dist(g4_x, g4_y, -2260, -940) <= 200 && money >= 20000) {
      combat_boss = true;
      money -= 20000;
      current_ILD_level = 100;
      gameStatement = 6;
    }
    ...
    if (menu == 1) { //MENU BATIMENT 1 OUVERT
      ...
      fill(255);
      text("X:"+g4_x+" Y:"+g4_y, 10, 40); //DEBUG INFO
      fill(0);
      textSize(60);
  ...
  if (gameStatement == 6) { //COMBAT TERRESTRE
    ...
    if (combat_boss == true) {
      ...
      background(255);
      image(jungle, 0, 0);
      image(pirate_vertical, pirate_T_x, pirate_T_y);
      image (boss, boss_x, boss_y);
      fill(255);
      rect(pirate_T_x-35, pirate_T_y-50, 250, 34);
      fill(210, 0, 0);
      rect(pirate_T_x-32, pirate_T_y-47, player_sante*1.22, 28);

      fill(255);
      rect(boss_x, boss_y-100, 320, 68);
      fill(210, 0, 0);
      rect(boss_x+3, boss_y-97, boss_sante*0.1256, 62);
      ...
      if (turn_boss == false) {
        ...
        if (pirate_T_x >= 1000) {
          delay(200);
          boss_sante -= 60 + boost_degats;
          pirate_T_x = 80;
          pirate_T_y = 450;
          player_animate = false;
          turn_boss = true;
        }
        if (player_animate == true) {
          if (frameCount%6 == 0) {
            pirate_T_x += 30;
          }
        }
      ...
      if (frameCount%60 == 0 && player_sante < 200) {
        player_sante += 1;
      }
      if (player_sante <= 0) {
        gameStatement = 4;
      ...
      if (boss_sante <= 0) {
        int score = frameCount/30;
        background(255);
        textSize(84);
        text("VOUS AVEZ GAGNE ! FELICITATIONS !", 50, 600);
        textSize(30);
        text("VOTRE SCORE: "+score+"", 60, 700);
      }
    } else {
      ...
      if (T_tour_ennemi == false) { //A NOTRE TOUR DE JOUER
        if (mousePressed == true) {
          if (current_ILD_level == 1 || current_ILD_level == 2 || current_ILD_level == 3) {
